...
echo 'Enter password for emugif2 (escape any special characters with 3 backslashes):'
read -s pass
...
do
	...
	../"$dir1"autopass.sh "$pass" "scp localuser@emugif2.cern.ch:/mnt/backup/raid/data/current/TMBCounters_GIF++_Test40_${datestr}_UTC.txt "m${meas}_"$chamber"_$(awk '/'$meas'/ {print $3}' ../"$dir1"meta)_test40_SRCoff_$(awk 'BEGIN{FS="_"; OFS="_"} /^'$meas'/ {print $7,$8}' ../"$dir1"meta)_TMBCounters.txt
